...
def generate_synthetic_sequence(
    ...
    :param  man_dir: manoeuvre direction, -1 for left, 0 for center, 1 for right
    ...
    print(start_offset)
    start_offset = np.array(start_offset)

    target_location[0] += int(man_dir*w/(4*max_zoom))
    start_offset[0] += int(man_dir*w/(4*max_zoom))
    ...
    for cur_index in range(seq_len):
        ...
        if lateral_motion:
            ...
            y_offset = 0.0#int(start_offset[1] * seq_frac)
