...
def generate_synthetic_sequence(
    ...
    :param image: base image to augment
    :param target_location: x, y coords of target in image
    :param  man_dir: manoeuvre direction, -1 for left, 0 for up, 1 for right
    ...
    :param turn_channel: Yaw or roll, ie whether we drive like a car or with 3D translations only
    :return:
    """
    ...
    up_offset = target_location[1] - h/(3*max_zoom)
    left_offset = target_location[0] - w/(3*max_zoom)
    right_offset = -w + target_location[0] + w/(3*max_zoom)

    # Apply manoeuvre over new number of steps
    new_len = int(seq_len/2)
    for cur_index in range(new_len):
        yaw_command = 0
        pitch_command = 0
        throt_command = 0
        roll_command = 0
        ...
        # goes down linearly from 1 to 0 as cur_index increases, stays at 0 during static frames (can't go negative)
        # seq_frac = max(0.0, (seq_len - cur_index) / seq_len)
        seq_frac = cur_index / new_len

        if lateral_motion:
            if man_dir == -1:
                # compute crop linearly based on offset
                x_offset = int(left_offset * seq_frac)
                y_offset = 0
            elif man_dir == 0:
                # compute crop linearly based on offset
                x_offset = 0
                y_offset = int(up_offset * seq_frac)
            elif man_dir == 1:
                # compute crop linearly based on offset
                x_offset = int(right_offset * seq_frac)
                y_offset = 0
            else:
                raise ValueError(f"Unsupported manoeuvre ID {man_dir}")
            # Coords move target in direction of PIL axes (ex +x moves target right, +y moves target down). Note this
            # moves the crop borders in the opposite direction
            crop_tuple = compute_crop_offsets(frame_center=target_location, crop_size=crop_size,
                                              offset=(x_offset, y_offset))
        ...
        out_seq.append(aug_img)
        # calculate control signal
        control_outputs.loc[cur_index + seq_len + num_static_frames, "vx"] = pitch_command
        control_outputs.loc[cur_index + seq_len + num_static_frames, "vy"] = roll_command
        control_outputs.loc[cur_index + seq_len + num_static_frames, "vz"] = throt_command
        control_outputs.loc[cur_index + seq_len + num_static_frames, "omega_z"] = yaw_command  # always 0 for now
...
def get_synthetic_params(image_path: str, target_location: Sequence[int], man_dir: int,
    ...
    img = img.resize([256,144], resample=Image.BICUBIC)
